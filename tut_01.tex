...
\begin{itemize}
  ...
  \item Columns: \texttt{email}, \texttt{name}, \texttt{faculty}, \texttt{department}, \texttt{joined}
...
The database stores:
\begin{itemize}
  \item \textbf{Students}: name, faculty, department, \texttt{joined} date, graduation date. Identifier: \texttt{email}.
...
\begin{itemize}
  ...
  \item \textbf{NOT NULL} \texttt{student.name, joined, faculty, department} — required student information cannot be missing.
  \item \textbf{CHECK} \texttt{book.format = 'paperback' OR 'hardcover'} — only permitted book formats are stored.
  \item \textbf{CHECK} \texttt{student.graduate >= joined} — a graduation date cannot be before the student joined.
...
\begin{lstlisting}[language=SQL]
INSERT INTO student (email, name, joined, faculty, department) VALUES (
  'ben.lim@u.nun.edu',
  'BEN LIM',
  '2026-08-10',
  'School of Computing',
