...
FINAL="$DIR/relational_algebra.mp4"
...
cp "$FINAL" "$SITE_DIR/tut_01_relational_algebra.mp4"

# Grab a thumbnail from the Selection scene, where the table highlight
# and the RA/SQL panel are both clearly visible.
ffmpeg -y -ss 00:00:25 -i "$FINAL" -frames:v 1 -vf "scale=480:-1" \
  "$SITE_DIR/thumbs/tut_01_relational_algebra.png"
