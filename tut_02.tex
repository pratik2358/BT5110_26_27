...
\node[erwire, fit=(opened), inner xsep=7mm, inner ysep=2mm] (aggbox) {};

% ---- Right column: contain -- wine ----
\node[errel] (contain) at (10.4,-3.4) {contain};
\node[erentity, minimum width=5.2cm] (wine) at (10.4,-6.8) {wine};
...
\coordinate (num-junction) at ($(bottle.north)+(0.7,0.6)$);
\coordinate (num-attr) at ($(bottle.north)+(0.7,0.9)$);
\draw[erwire] ($(bottle.north)+(0.7,0)$) -- (num-junction);
\node[erfilled] (num-dot) at (num-junction) {};
\draw[erwire] (num-junction) -- (num-attr);
\node[erhollow] (num-leaf) at (num-attr) {};
\node[erlabel, above=1mm of num-leaf] {Number};

% Number is scoped by wine (weak entity): route down to wine's corner
\coordinate (wine-junction) at ($(wine.north)+(0,0.2)$);
\draw[erwire] (num-dot.east) -- ++(1.1,0) |- (wine-junction);
\draw[erwire] (wine-junction) -- (wine.north);
\node[erfilled] at (wine-junction) {};

% ---- wine attributes: Others, W.Name / Appellation / Vintage (composite) ----
\coordinate (others-attr) at ($(wine.north)+(-1.3,0.9)$);
\draw[erwire] ($(wine.north)+(-1.3,0)$) -- (others-attr);
